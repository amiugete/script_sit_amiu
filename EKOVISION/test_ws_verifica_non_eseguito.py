...
def main():
    ...
    params={'obj':'schede_lavoro',
        'act' : 'r',
        'sch_lav_data': f'{data_text}',
        'cod_modello_srv': f'{cod_percorso}',
        'flg_includi_eseguite': 1,
        'flg_includi_chiuse': 1
        }
        ...
        if len(letture['schede_lavoro']) == 0:
            ...
            while ss < len(letture['schede_lavoro']):
                ...
                if int(letture2['schede_lavoro'][0]['servizi'][0]['flg_segn_srv_non_effett'])==1:
                    ...
                    testo_warning=f'''La scheda {id_scheda} (cod_percorso = {cod_percorso} del {data_text}) risulta non effettuata (id_causale ekovision = {id_causale_eko}). 
Le consuntivazioni che arrivano da totem saranno ignorate'''
