...
def main():
    ...
    ms0= (oggi - datetime.timedelta(days=7))
    starttime= int(time.mktime(ms0.timetuple())) #* 1000
    ...
    """
    # recupero lo starttime tornando indietro di 7 giorni (da vedere quanto ci mette)
    query_st='''select extract(epoch from (now() at time zone 'cet')::date - interval '7' day)::int as starttime,
extract(epoch from now() at time zone 'cet')::int as endtime '''
    ...
    VALUES(%s, %s, %s, %s);'''
            ...
            query_upsert='''INSERT INTO spazz_bucher.messaggi
                (routeid, driverid, sweeper_mode, geoloc, sportello, data_ora, trk_id)
                VALUES(%s, %s, %s, ST_SetSRID(ST_MakePoint(%s, %s),4326), %s, %s, %s)
                ON CONFLICT (sportello, data_ora)
                DO UPDATE set routeid =%s, driverid =%s, sweeper_mode =%s,
                geoloc=ST_SetSRID(ST_MakePoint(%s, %s),4326), trk_id=%s,
                data_ultima_modifica = now();'''
            try:
                curr.execute(query_upsert, (route_id, driver_id, sweeper_mode, lon, lat, sportello, s_time, trk_id,
                                            route_id, driver_id, sweeper_mode, lon, lat, trk_id,))
            except Exception as e:
                logger.error(e)
