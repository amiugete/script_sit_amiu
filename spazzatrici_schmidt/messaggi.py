...
def main():
    ...
    # per ora escludo la spazzatrice 100001111 che da dei problemi e la tratto a parte

    query_select="""SELECT id 
        FROM spazz_schmidt.serialnumbers 
        where api_attive = 't' 
        and id not in ('100001111', '100001114')"""
    try:
            ...
            if lastid>0 and check <1:
                ...
                try:
                    ...
                    while i<len(letture):
                        ...
                        # bypasso questo controllo con un ON CONFLICT (id) DO NOTHING spero vada più veloce
                    ...
                    logger.warning(err)
                    logger.warning('Problema lettura messaggi della spazzatrice con SN {0} '.format(sn[0]))
