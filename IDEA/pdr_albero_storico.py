...
def main():
    ...
    while check<1:
        ...
        #response = requests.get(api_url, params={'id_pdr':'24667', 'page_size': 100, 'page_index': p}, headers={'Authorization': 'Token {}'.format(token1)})
            ...
            while i < len(letture['data']):
                ...
                try:
                    if float(letture['data'][i][0]['lat'])>0:
                        ...
                        try: 
                            id_pdr_int=int(id_pdr)
                        except Exception as e2:
                            check_pdr_intero=1
                            logger.warning(f"{id_pdr} - Non ha id_piazzola AMIU, potrebbe essere un magazzino (MAG...) oppure contenitori non censiti") 
                        ...
                        if check_pdr_intero==0 or id_pdr.startswith("MAG"):
                            if letture['data'][i][0]['contenitori'] == None:
                                logger.warning(f'Nel PdR {id_pdr} non ci sono contenitori')
                            else:    
                                # ciclo su contenitori
                                j=0
    ...
    if len(nuovi_id_ok)>0:
        logger.info('Ci sono nuove piazzole o sono cambiati i contenitori nelle piazzole- Predispongo il file e mando mail')
        # Imposto file con nuove piazzole
        file_piazzole="{0}/output_file/{1}_piazzole_nuove_aggiornate.xlsx".format(path, giorno_file)
    ...
    if len(old_id_pdr_ok)>0:
        ...
        file_piazzole2="{0}/output_file/{1}_contenitori_rimossi.xlsx".format(path, giorno_file)
