...
'''
Lo script verifica le anomalie inviate da ekovision
'''
...
f = open("{}/analisi_anomalie_20240516.csv".format(path), "w")
...
def main():
    ...
    f.write("riga;codice_servizio;codice_comp o codice_tratto;data_inizio_corretta\n")
    
    
    query='''
    SELECT codice as codice_modello_servizio,
    coalesce(cod_comp, cod_tratto) as codice, 
    data_inizio, data_fine, "data_inizio-2", 
    "data_fine-2" 
    FROM marzocchir.temp_amiu_controllo_itinerari4;
        '''
    ...
    order by codice_modello_servizio, data_fine desc, ordine,  ripasso'''
        ...
        for dpe in dettaglio_percorsi_ekovision:
            if int(dpe[12])==int(vv[2]) or int(dpe[12])==int(vv[4]):
                date_inizio.append('{} rip {}'.format( dpe[12], dpe[14]))
            
        '''if int(vv[2]) in date_inizio or int(vv[4]) in date_inizio:
            logger.debug('OK')
        else:'''
        percorso_attivo='''select * from elem.percorsi p 
            where cod_percorso = %s 
            and id_categoria_uso in (3,6)'''
        
        try:
            #curr.execute(query_variazioni_ekovision,(cod_percorso,))
            curr.execute(percorso_attivo,(vv[0],))
            check_percorso_attivo=curr.fetchall()
        except Exception as e:
            logger.error(e)
            check_ekovision=101 # problema query
        if len(check_percorso_attivo)==1:
            logger.warning('''Riga {} - Problema percorso {} elemento {}. 
                            Data inizio EKV {} - Data inizio itinerari {}'''.format(
                                riga, vv[0], vv[1], vv[2], date_inizio
                            ))
            f.write('{};{};{};{}\n'.format(riga, vv[0], vv[1], date_inizio))
        else:
            logger.debug('Percorso disattivo')
            f.write('{};{};PERCORSO DISATTIVO\n'.format(riga, vv[0]))
