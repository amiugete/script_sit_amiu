...
def checkFileArchive(cursor, logger):
    ...
    try:
        cursor.execute(query["get_last_file"])
        db_data = cursor.fetchone()
        db_year = int(db_data[0])
        db_week = int(db_data[1])
    except Exception as e:
        logger.error("Errore nel recupero dell'ultimo file caricato")
        logger.error(e)

    if (db_year == year and (week - db_week) >=2) or (db_year == (year -1) and (db_week <= 52 and week >=2)):
        return False, db_year, db_week
    else:
        return True, db_year, db_week
