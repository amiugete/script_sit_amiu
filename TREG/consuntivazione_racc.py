...
FROM treg_eko.last_import_treg_racc where commit_code=200 and deleted = false'''
    ...
    data_start=datetime.strptime('20250101', '%Y%m%d').date() #per eventuale debug
    
    fine_ciclo=oggi
    fine_ciclo = datetime.strptime('20250102', '%Y%m%d')
    #fine_ciclo=date(fine_ciclo.year, fine_ciclo.month, fine_ciclo.day)
    
    
    
    #### !!! CABLATA NEL CODICE E VA MODIFICATA MANUALMENTE IN BASE ALL'ANDAMENTO DI CHIUSURA SCHEDE
    # Prima di questa data controllo che tutte le schede siano chiuse 
    data_controllo= datetime.strptime('20250731', '%Y%m%d')
    ...
    while  data_start <= fine_ciclo:
        ...
        select cod_percorso, versione_testata, fo.freq_binaria, freq_settimane, id_turno, at2.gestione_arera, 
            to_date(cc.data_pianif_iniziale, 'YYYYMMDD') as data_pianif_iniziale, cc.time_update, ep.descrizione
            ...
            left join (select distinct codice_servizio_pred, data_pianif_iniziale, max(ce.data_last_update) as time_update
                from treg_eko.consunt_ekovision ce
                group by codice_servizio_pred, data_pianif_iniziale
            ) cc on ep.cod_percorso = cc.codice_servizio_pred
        ...
        for ep in elenco_percorsi:
            ...
            if tappa_prevista(data_start,  ep[2])==1 and (ep[3]=='T' or ep[3]==check_s): 
                ...
                logger.debug('Verifico che esista una scheda consuntivata per quella data (data_pianif_iniziale) e per quel percorso con quel turno previsto')
                if (data_start <= data_controllo and ep[6] is None):
                    messaggio_errore=f'''ATTENZIONE URGENTE! 
                    Su Ekovision il giorno {ep[6]} la scheda del percorso {ep[0]} - {ep[8]} potrebbe essere ancora aperta o peggio non esistere. 
                    Nel primo caso si prega di chiudere la scheda, nel secondo di crearla, quindi eseguirla e chiuderla.')'''
                    warning_message_mail(messaggio_errore, 'assterritorio@amiu.genova.it', os.path.basename(__file__), logger)
                else: 
...
-- PRIMO STEP PER TOGLIERE I RIPASSI (al secondo step dovrò considerare le possibili schede doppie)
	...
	case
		...
		where ce1.id_scheda_riprogr = ce.id_scheda_riprogr)
	...
	where ce.tipo_servizio = 'RACC'
	and codice_servizio_pred = '0501014509'
and data_pianif_iniziale = '20250127'
	group by 
	case
		when flg_riprogrammato = 0 then id_scheda
		else id_scheda_riprogr
...
select  
	...
	case
		...
		else max(distinct gs.causale::int)
	end causale, 
	...
	codice, ep2.giorno_competenza
            ...
            for eep in elenco_elementi_percorso:
                ...
                wasteCollection={
                    ...
                    'programmingStartDate':programming_start_ending_date(curr1, data_start, t, eep[6])[0],
                    'programmingEndingDate':programming_start_ending_date(curr1, data_start, t, eep[6])[1],
